...
>
> Layers: `src/optics/` (pure, unit-tested physics) → `src/lab/` (lens descriptions, illustrative optical layouts) →
> `src/scene/` + `src/render/` (three.js bench, sensor pipeline, post-processing) → `src/ui/` (DOM panels; the library,
> phones and compare views are lazy chunks). Product data is JSON in `data/`, loaded per file with `import.meta.glob`.
> Earlier decisions are recorded in `PLAN.md` ("Key decisions", "Phase 2 decisions"); add new ADRs there or in `docs/`.
> The `system-design` skill mentioned below was not copied into this repo.
