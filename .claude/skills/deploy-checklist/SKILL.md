...
>
> Deploying = merging to `main`: `.github/workflows/deploy.yml` runs `npm ci`, `npm test` and `npm run build`, then
> publishes `dist/` to GitHub Pages (https://hazemalhayattrading.github.io/lens-lab/). There are no migrations, feature
> flags, canaries or on-call. Before merging: tests and build green, `npm run preview` works under the `/lens-lab/` base
> path, no console errors, lazy chunks load, bundle sizes did not jump. Rollback = revert the merge commit on `main`
> (Pages redeploys the previous build).
