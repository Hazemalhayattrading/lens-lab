...
>
> "Staging vs prod" here means `npm run dev` vs the production build: reproduce with `npm run build && npm run preview`
> (served under the `/lens-lab/` base path, like https://hazemalhayattrading.github.io/lens-lab/). Collect the browser
> console and WebGL warnings; cloud sessions have Chromium + Playwright preinstalled (`scripts/shots.mjs` takes
> screenshots). Usual suspects: asset URLs that ignore the base path, a lazy chunk that fails to load, three.js resources
> used after being disposed on a lens swap, shader compile errors, NaN / ∞ from optics edge cases (focus at or inside f,
> objects at ∞). "Recent changes" = `git log`; deploy runs are in GitHub Actions (`.github/workflows/deploy.yml`).
